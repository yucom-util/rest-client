...
import { AppError, txid } from '@yucom/common';
...
const log = Log.create('http-client');
...
class Client {
  ...
  async request(req: HttpRequest): Promise<any> {
    log.debug('Starting ' + req.description);
    let resBody;
    try {
      resBody = await execute(this.axios, req);
      log.debug(`${req.description} succeded`);
    } catch (err) {
      const appError = buildAppError(err, req);
      log.error(`${req.description} FAILED.`, appError);
      throw appError;
    }
    return resBody;
  }
